...
const static unsigned char reg_t8[] ={23,0,3,1,0,0,0,0,0,0};
const static unsigned char reg_t9[] ={143,0,0,14,8,0,32,46,2,3,0,20,2,64,4,10,25,0,0,0,0,0,1,1,45,45,234,53,231,85,10,10,0,0,2};
...
#else 
const static unsigned char reg_t8[] ={23,0,1,1,0,0,0,0,0,0};
...
#ifdef CONFIG_TARGET_PRODUCT_C5155
const static u16 touch_field_y[] = {0,910,970,1018,1023};
#else 
...
static void touch_change_state(int new_sleep, int new_mode)
{
...
static void touch_early_suspend(struct early_suspend *h)
{
  ...
  touch_change_state(TOUCHPANEL_SLEEP_STATE_SUSPEND,ts_data->mode_state);
...
static void touch_early_resume(struct early_suspend *h)
{
  ...
  mutex_lock(&tp_chg_mode_lock);
  touch_change_state(TOUCHPANEL_SLEEP_STATE_RESUME,ts_data->mode_state);
  mutex_unlock(&tp_chg_mode_lock);
...
static void touch_patent_enable(struct select_dev *dev, int value)
{
  if(value!=0)
  {
    ...
    {
      touch_change_state(ts_data->sleep_state,TOUCHPANEL_MODE_STATE_PATENT);
      mutex_unlock(&tp_chg_mode_lock);
    }
  }
  else
  {
    if(!mutex_lock_interruptible(&tp_chg_mode_lock))
    {
      touch_change_state(ts_data->sleep_state,TOUCHPANEL_MODE_STATE_NORMAL);
      mutex_unlock(&tp_chg_mode_lock);
    }
  }
}
